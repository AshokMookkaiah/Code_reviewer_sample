...
# --- metrics (PostgreSQL via DuckDB — no extra pip packages) ---
...
class ReviewMetrics:
    ...
    webhook_event_uuid: str | None = None


def _findings_section(review_markdown: str) -> str:
    """Prefer the ## Findings block; fall back to full text."""
    text = review_markdown or ""
    match = re.search(r"(?im)^##\s*findings\s*$", text, re.MULTILINE)
    if not match:
        return text
    start = match.end()
    rest = text[start:]
    next_heading = re.search(r"(?im)^##\s+\S", rest, re.MULTILINE)
    return rest[: next_heading.start()] if next_heading else rest


def _count_severity_tags(section: str, severity: str) -> int:
    """
    Count lines like:
      - [CRITICAL] src/app.py:5 — description
      * [WARNING] path:line — ...
      [INFO] path:line — ...   (no bullet)
    """
    # Optional bullet (- * + •) or numbered list; then [SEVERITY]; then path:line
    pattern = (
        rf"(?im)(?:^|\n)\s*"
        rf"(?:(?:[-*+•]\s+)|(?:\d+\.\s+))?"
        rf"\[\s*{severity}\s*\]"
...
def count_findings(review_markdown: str) -> tuple[int, int, int]:
    """Count [CRITICAL] / [WARNING] / [INFO] finding lines in review Markdown."""
    section = _findings_section(review_markdown)

    critical = _count_severity_tags(section, "critical")
    warning = _count_severity_tags(section, "warning")
    info = _count_severity_tags(section, "info")

    if critical + warning + info:
        return critical, warning, info

    # Legacy / alternate formats
    critical = len(re.findall(r"(?im)(?:^|\n)\s*[-*+]\s+.*\*\*\s*critical\s*\*\*", section))
    warning = len(re.findall(r"(?im)(?:^|\n)\s*[-*+]\s+.*\*\*\s*warning\s*\*\*", section))
    info = len(re.findall(r"(?im)(?:^|\n)\s*[-*+]\s+.*\*\*\s*info\s*\*\*", section))

    try:
        payload = json.loads(review_markdown)
...
def _database_url_to_libpq(url: str) -> str:
    parsed = urlparse(url)
    if parsed.scheme not in ("postgresql", "postgres"):
        raise ValueError("DATABASE_URL must use postgresql://")
    query = dict(urllib.parse.parse_qsl(parsed.query))
    sslmode = query.get("sslmode", "require")
    dbname = (parsed.path or "/").lstrip("/") or "defaultdb"
    parts = [
...
def _metrics_mode() -> str:
    return (_env("METRICS_MODE", "postgres") or "postgres").strip().lower()


def _metrics_ingest_url() -> str:
    return (_env("METRICS_INGEST_URL") or "").strip()


def _metrics_ingest_token() -> str:
    return _env("METRICS_INGEST_TOKEN") or ""


def _metrics_row_dict(metrics: ReviewMetrics) -> dict:
    return {
        "project_id": metrics.project_id,
        "project_path": metrics.project_path,
        "mr_iid": metrics.mr_iid,
        "mr_title": metrics.mr_title,
        "mr_url": metrics.mr_url,
        "event_action": metrics.event_action,
        ...
        "findings_warning": metrics.findings_warning,
        "findings_info": metrics.findings_info,
        "gitlab_note_id": metrics.gitlab_note_id,
        "error_message": metrics.error_message,
        "webhook_event_uuid": metrics.webhook_event_uuid,
    }


def _record_metrics_http(metrics: ReviewMetrics) -> bool:
    url = _metrics_ingest_url()
    if not url:
        return False
    headers = {"Content-Type": "application/json"}
    token = _metrics_ingest_token()
    if token:
        headers["X-Metrics-Token"] = token
    body = json.dumps(_metrics_row_dict(metrics)).encode("utf-8")
    ...
    try:
        ...
        print(f"HTTP metrics ingest failed: {exc}")
        return False


def _record_metrics_postgres(metrics: ReviewMetrics) -> bool:
    ...
    try:
        ...
        print("METRIC_JSON:" + json.dumps(metrics.__dict__, default=str))
        return False

    conn_str = _database_url_to_libpq(database_url).replace("'", "''")
    con = duckdb.connect()
    try:
        con.execute("INSTALL postgres; LOAD postgres;")
        con.execute(f"ATTACH '{conn_str}' AS metrics_db (TYPE postgres)")
        row = _metrics_row_dict(metrics)
        con.execute(
            """
            INSERT INTO metrics_db.public.code_review_runs (
                project_id, project_path, mr_iid, mr_title, mr_url,
                event_action, review_status, latency_ms, model_id,
                prompt_tokens, completion_tokens, total_tokens,
                findings_critical, findings_warning, findings_info,
                gitlab_note_id, error_message, webhook_event_uuid
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                row["project_id"],
                ...
                row["findings_warning"],
                row["findings_info"],
                row["gitlab_note_id"],
                row["error_message"],
                row["webhook_event_uuid"],
            ],
        )
        return True
    except Exception as exc:
        print(f"PostgreSQL metrics insert failed: {exc}")
        print(
            "Hint: standalone Functions cannot be DB trusted sources — see docs/METRICS-NETWORKING.md"
        )
        return False
    finally:
        con.close()
...
def record_metrics(metrics: ReviewMetrics) -> None:
    mode = _metrics_mode()
    if mode == "off":
        return
    ...
    if mode in ("postgres", "both"):
        ok = _record_metrics_postgres(metrics) or ok
    if mode in ("http", "both"):
        ok = _record_metrics_http(metrics) or ok

    if not ok and mode == "postgres":
        print("METRIC_JSON:" + json.dumps(metrics.__dict__, default=str))
